...
class Pix_Controller
{
	...
	return call_user_method_array($func, $plugin, $args);
    }
	...
	try {
	    ...
	    $file = $controller->view->getPath() . "$controllerName/$actionName.phtml";
	    if (file_exists($file)) {
		echo $controller->draw("$controllerName/$actionName.phtml");
	    } else {
