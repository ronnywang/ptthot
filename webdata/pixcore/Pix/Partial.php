...
class Pix_Partial
{
	...
	if (array_key_exists('_data', $data)) {
        ...
        $cache_key = sprintf('Pix_Partial:%s:%s:%d',
                sha1(file_get_contents($path)),
                $cache_id,
                self::$_trim_mode ? 1 : 0);
        if (strlen($cache_id) > 0 and $html = $cache->load($cache_key)) {
            return $html;
        }
	...
	return call_user_method_array($func, $helper, $args);
    }
}
