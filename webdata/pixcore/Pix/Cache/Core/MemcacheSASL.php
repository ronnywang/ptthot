...
require_once(__DIR__ . '/../../../extlibs/MemcacheSASL.php');

/**
 * Pix_Cache_Core_MemcacheSASL
 * 
 * @uses Pix_Cache_Core
 * @package Pix_Cache
 * @version $id$
 ...
 * @license 
 */
class Pix_Cache_Core_MemcacheSASL extends Pix_Cache_Core
{
	...
	if (is_null($this->_memcache)) {
	    ...
	    $this->_memcache = new MemcacheSASL;
		...
		$this->_memcache->addServer(
		    ...
		    isset($server['weight']) ? intval($server['weight']) : 1
                ...
                if ($server['user']) {
                    $this->_memcache->setSaslAuthData($server['user'], $server['password']);
                }
	    }
	}
