...
class Pix_Table_Row
{
	...
	if ($row instanceof Pix_Table_Row) {
		...
		throw new Exception('這兩個是不同的 ModelRow ，你的程式是不是寫錯了?');
	    }
    ...
    public function update_by_string($args)
    {
        try {
            $this->preSave();
    ...
    {
        if (!is_array($args)) {
            return $this->update_by_string($args);
	}
	$table = $this->getTable();

	foreach ($args as $column => $value) {
	    if ($table->_relations[$column]) {
		if ($value instanceof Pix_Table_Row) {
		    $this->{$column} = $value;
		}
	    } elseif ($table->_columns[$column]) {
		$this->{$column} = $value;
	    } elseif ($table->_hooks[$column] and isset($table->_hooks[$column]['set'])) {
		$this->{$column} = $value;
	    } elseif ('_' == $column[0]) {
		$this->{$column} = $value;
	    }
	...
	if (!$this->_primary_value) { 
	    throw new Exception('這個 Row 還不存在在 DB 上面，不能刪除');
        }
	...
	if (!isset($conf['tableClass'])) {
	    throw new Exception('建立 Row 必需要指定 tableClass');
	}
	...
	if (isset($conf['data'])) {
		...
		if (!isset($conf['data'][$column])) {
                    throw new Exception("{$this->_tableClass} Row 的資料抓的不完整(缺少 column: {$column})");
		}
...
/*		    if (strlen($value) > $column['size'])
	...
	if (!$get = $this->getTable()->_hooks[$name]['get']) {
	    throw new Exception("沒有指定 {$name} 的 get 變數");
	}
    ...
    public function getRelation($name, $notnull = true)
    {
	$table = $this->getTable();
	if (isset($this->_relation_data[$name])) {
        ...
        } elseif ($table->_relations[$name]['notnull'] and $notnull) {
            try {
                $this->_relation_data[$name] = $row = $this->createRelation($name);
            } catch (Pix_Table_DuplicateException $e) {
                $this->_relation_data[$name] = $row = $this->getRelation($name, false);
            }
        } else {
            $row = null;
        }
	...
	if ($table->_relations[$name]['rel'] == 'has_many') {
	    throw new Exception("has_many 不能夠 ->{$name} = \$value; 請改用 ->{$name}[] = \$value");
	...
	if ($table->_relations[$name]) {
	    $row = $this->getRelation($name, false);
	    if ($row)
		return true;
	    else
		return false;
    ...
    public function createRelation($relation, $values = array())
    {
        ...
        if (!in_array($table->_relations[$relation]['rel'], array('has_one', 'belongs_to'))) {
            if (is_array($values)) {
                ...
                return $this->{$relation}->insert(array());
            }
        }
        ...
        foreach (array_combine($primary_keys, $foreign_values) as $key => $value) {
	...
	$table = $this->getTable();
	    ...
	    throw new Exception(get_class($this) . " 沒有 $name 這個 function 喔");
	}
    }
    ...
    public function refreshRowData()
    {
        ...
        if ($row = $this->getRowDb()->fetchOne($table, array_combine($table->getPrimaryColumns(), $this->getPrimaryValues()))) {
