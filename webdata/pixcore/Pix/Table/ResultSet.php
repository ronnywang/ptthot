...
class Pix_Table_ResultSet extends Pix_Array // implements Pix_Array_Volumable
{
	...
	if (isset($conf['where'])) {
            $this->_search_object = Pix_Table_Search::factory($conf['where'], $this->getTable());
        } else {
    ...
    {
        $this->_search_object->offset($offset);
	return $this;
    }

    public function index($index = null)
    {
        $this->_search_object->index($index);
	return $this;
    }
    ...
    {
        $this->_search_object->order($order);
	return $this;
    }

    public function limit($limit = null)
    {
        $this->_search_object->limit($limit);
	return $this;
    }
    ...
    public function rewind($select_columns = '*')
    {
        ...
        if ($where and 'PRIMARY' == $this->getTable()->findUniqueKey(array_keys($where))) {
	    $val = array();
	...
	$rs = clone $this;
        $rs->_rowset = null;
        $rs->_search_object = clone $this->_search_object;
        $rs->_search_object->search($where);
	return $rs;
    }

    public function after($row = null)
    {
	$rs = clone $this;
	$rs->_rowset = null;
        $rs->_search_object = clone $this->_search_object;
        $rs->_search_object->after($row);
	return $rs;
    }

    public function before($row = null)
    {
	$rs = clone $this;
	$rs->_rowset = null;
        $rs->_search_object = clone $this->_search_object;
        $rs->_search_object->before($row);
	return $rs;
    }
    ...
    {
	throw new Exception('TODO: Pix_Table 未實作此功能，請使用 Pix_Table_Cluster');
    }
	...
	$rs = clone $this;
        return $rs->order(array($column => 'desc'))->offset(0)->limit(1)->rewind()->current();
    }
	...
	$rs = clone $this;
        return $rs->order(array($column => 'asc'))->offset(0)->limit(1)->rewind()->current();
    }

    public function first()
    {
	$rs = clone $this;
	return $rs->limit(1)->current();
    }
	...
	throw new Exception("找不到 function {$func}");
    }
