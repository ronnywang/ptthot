 ...
 */
class Pix_Table_Db_Adapter_SQL implements Pix_Table_Db_Adapter
{
    /**
     * fetchOne 從 $table 找出符合 $search 條件的一筆
     * 
     ...
     */
    public function fetchOne($table, $search)
    {
        $sql = 'SELECT * FROM ' . $this->column_quote($table->getTableName());
        if (is_scalar($search)) {
            $sql .= $search;
        } else {
            $search = Pix_Table_Search::factory($search, $table);
            $sql .= ' WHERE ';
    ...
    public function fetchCount($table, $search)
    {
        ...
        if (is_scalar($search)) {
            ...
            $search = Pix_Table_Search::factory($search);
            $sql .= ' WHERE ';
            $sql .= $this->_get_where_clause($search);
        }
