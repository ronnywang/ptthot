...
class Pix_Table_Db_Adapter_Sqlite extends Pix_Table_Db_Adapter_SQL
{
	...
	foreach ($table->_columns as $name => $column) {
		...
		$s .= '(' . $column['size'] . ')';
            ...
            // http://bug.mgmt.pixnet/trac/ticket/18074 在 varchar, char, text 加上 COLLATE NOCASE, 比較時忽略大小寫
	...
	$sql .= " (\n" . implode(", \n", $column_sql) . ") \n";
        ...
        if (is_array($table->_indexes)) {
            foreach ($table->_indexes as $name => $index) {
                $s = 'CREATE INDEX ' . $this->column_quote($table->getTableName() . '_' . $name) . ' ON ' . $this->column_quote($table->getTableName());
                $index_columns = array();
                foreach ($index as $column_name) {
                    $index_columns[] = $this->column_quote($column_name);
                }
