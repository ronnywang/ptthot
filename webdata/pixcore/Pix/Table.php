...
require_once(dirname(__FILE__) . '/Table/Row.php');
require_once(dirname(__FILE__) . '/Table/ResultSet.php');
...
abstract class Pix_Table
{
	...
	if (!is_null($table)) {
		...
		if (!is_scalar($table) or !class_exists($table)) {
		    throw new Exception("找不到 {$table} 這個 class ，請確認是否有 link 到 classes 或者是有打錯字");
		}
    ...
    {
        if (!defined('PIX_TABLE_DROPTABLE_ENABLE')) {
            throw new Exception('要 DROP TABLE 前請加上 PIX_TABLE_DROPTABLE_ENABLE');
        }
	$table = self::getTable($this);
	$table->getDb()->dropTable($table);
    }
	...
	$conf['primary_value'] = $primary_value;
        ...
        if (!$row = $table->getDb()->fetchOne($table, array_combine($table->getPrimaryColumns(), $primary_value))) {
    ...
    /**
     * getPrimaryValues
     * 取得某個 Row 的 Primary Value ，$row 可以是 primary_value 或是 Pix_Table_Row
     * 
     * @param mixed $row 
     * @access public
     * @return array
     */
    public function getPrimaryValues($row)
    {
	$primary = $this->getPrimaryColumns();

        if ($row instanceof Pix_Table_Row) {
            return $row->getPrimaryValues();
        } elseif (is_scalar($row) && count($primary) == 1) {
            return array($row);
        } elseif (is_array($row) && count($row) == count($primary)) {
            return $row;
	} elseif ($row === null) {
	    return null;
        } else {
	    throw new Exception('必需是 Pix_Table_Row('.$this->getClass().') 或是 Primary Value');
        }
    }
	...
	if (!class_exists($plugin)) {
	    throw new Exception("Plugin {" . $plugin . "} is not found");
	}
    ...
    {
	return $this->_plugins[$method]['plugin_method'] ? $this->_plugins[$method]['plugin_method'] : $method;
    }

    public function getPlugin($method)
    {
	if (!$plugin = $this->_plugins[$method]) {
	    return null;
	}
	$class = $plugin['class'];
	if (!$plugin = $this->_plugin_pools[$class]) {
	    if (!class_exists($class)) {
		throw new Exception("在取得 " . $this->getClass() . " 的 {$method} plugins 時，找不到 {$class} class");
	    }
	    $this->_plugin_pools[$class] = $plugin = new $class($this->_plugin_class_options[$class]);
	    if (!($plugin instanceof Pix_Table_Plugin)) {
		throw new Exception("{$class} 不是 Pix_Table_Plugin");
	    }
	}
	return $plugin;
    }
	...
	throw new Exception("找不到這個函式喔: {$name}");
    }

    static public function preload($sets, $relation)
    {
	// 為了取得第一個 $row...
	foreach ($sets as $row) { break; }

	$table = $row->getTable();
	if (!$rel = $table->_relations[$relation]) {
	    throw new Exception("找不到 {$relation} 這個 relation");
	}
	if (!in_array($rel['rel'], array('has_one', 'belongs_to'))) {
	    throw new Exception("要 has_one 和 belongs_to 才能夠 preload");
	}
	$foreign_table = Pix_Table::getTable($rel['type']);
	$fk = is_array($rel['foreign_key']) ? $rel['foreign_key'][0] : $rel['foreign_key'];
	$pk = $table->getPrimaryColumns();
	$pk = $pk[0];
	...
	$foreign_table->search(1)->searchIn($pk, $ids)->rewind();
    }
    ...
    /**
     * setClass 設定這個 table 的 class name ，不指定預設會是 get_class 結果
     *
     * @param string $class
     * @access public
     * @return void
    ...
    static public function getRelationForeignTable($relation)
    {
        ...
        if (!$relation_data = $table->_relations[$relation]) {
            throw new Exception("{$this->getClass()} 找不到 {$relation} 這個 relation");
        }

        if (!$table_name = $relation_data['type']) {
            throw new Exception("{$this->getClass()}->{$relation} 沒有指定 Table Type");
        }
    ...
    static public function getRelationForeignKeys($relation)
    {
        $table = self::getTable($this);
        if (!$relation_data = $table->_relations[$relation]) {
            throw new Exception("{$table->getClass()} 找不到 {$relation} 這個 relation");
        }
        ...
        if (in_array($relation_data['rel'], array('has_one', 'belongs_to'))) {
            ...
            throw new Exception("{$table->getClass()}->{$relation} 的 rel 需要指定為 has_one 或 has_many");
        }

        throw new Exception("{$table->getClass()}->{$relation} 未指定 foreign_key");
    }
